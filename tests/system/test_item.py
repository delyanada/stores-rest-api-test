...
class ItemTest(BaseTest):
    ...
    def test_create_duplicates_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEquals(ItemModel.find_by_name('test').price, 17.99)
                self.assertEquals({'name': 'test', 'price': 17.99}, json.loads(resp.data))
    ...
    def test_put_update_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEquals(ItemModel.find_by_name('test').price, 5.99)
